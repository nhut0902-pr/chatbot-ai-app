...
object CerebrasService {
    private const val TAG = "CerebrasService"
    ...
    val api: CerebrasApi = retrofit.create(CerebrasApi::class.java)

    suspend fun fetchCompletion(
        request: ChatCompletionRequest,
        maxRetries: Int = 3
    ): ChatCompletionResponse {
        val apiKey = AppConfig.cerebrasKey
        val authHeader = "Bearer $apiKey"

        return retryWithBackoff(times = maxRetries) {
            Log.d(TAG, "Requesting Cerebras completion for model ${request.model}")
    ...
    private suspend fun <T> retryWithBackoff(
        ...
        repeat(times - 1) { attempt ->
            try {
                ...
                Log.w(TAG, "Cerebras attempt ${attempt + 1} of $times failed: ${e.message}")
